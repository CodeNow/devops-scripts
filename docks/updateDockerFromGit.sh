...
for DOCK in ${DOCKS[*]}; do
	...
	VERSION=`ssh ubuntu@$DOCK 'docker -H=127.0.0.1:4242 version | grep 63fe64c | wc -l'`
	...
	ssh ubuntu@$DOCK 'sudo apt-get install lxc-docker -y'
	...
	echo "resetart pm2"
	ssh ubuntu@$DOCK '/home/ubuntu/devops-scripts/docks/startDockletAndBouncer.sh production & echo "done"'
