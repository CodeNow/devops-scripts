...
# resource "aws_launch_configuration" "delta_lc_dock_pool" {
...
# resource "aws_autoscaling_group" "delta_asg_dock_pool" {
#   name                      = "${ lookup(var.dock_pool_asg_name, var.env) }"
#   launch_configuration      = "${ aws_launch_configuration.delta_lc_dock_pool.name }"
...
#   default_cooldown          = 0
#   desired_capacity          = "${ lookup(var.dock_pool_asg_desired, var.env) }"
#   force_delete              = true
...
# TODO: fix duplication due to envs
resource "aws_launch_configuration" "gamma_lc_dock_pool" {
  name                 = "${ lookup(var.dock_pool_ami_name, var.env) }"
  image_id             = "${ data.aws_ami.dock_pool_ami.id }"
  instance_type        = "${ lookup(var.dock_instance_type, var.env) }"
  iam_instance_profile = "${ lookup(var.dock_pool_iam_profile, var.env) }"
...
resource "aws_autoscaling_group" "gamma_asg_dock_pool" {
  name                      = "${ lookup(var.dock_pool_asg_name, var.env) }"
  launch_configuration      = "${ aws_launch_configuration.gamma_lc_dock_pool.name }"
  max_size                  = "${ lookup(var.dock_pool_asg_max_size, var.env) }"
  min_size                  = "${ lookup(var.dock_pool_asg_min_size, var.env) }"
  health_check_grace_period = 0
  health_check_type         = "EC2"
